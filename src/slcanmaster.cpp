...
slcanmaster::slcanmaster(const std::string &name, const YAML::Node &node)
    : module_base("module_slcan", name, node), runnable(node) 
{
...
void slcanmaster::set_state_init_2_preop() {
    ...
    if ((handle = can_init(CAN_BOARD(CANLIB_SERIALCAN, CANDEV_SERIAL),
                    CANMODE_DEFAULT, (const void *)&port)) < 0) {
        throw runtime_error(
                string_printf("interface could not be initialized: %d\n", handle));
    }

    bitrate.index = CANBTR_INDEX_250K;
    if ((result = can_start(handle, &bitrate)) < 0) {
        throw runtime_error(
                string_printf("interface could not be started: %d", result));
    }
}
...
void slcanmaster::set_state_preop_2_safeop() {
    for (const auto &stream_name : slave_stream_names) {
        ...
        if (!s) {
            throw runtime_error(
                    string_printf(" module not found %s\n", stream_name.c_str()));
        }
